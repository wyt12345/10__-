...
select * from sales where date between '2016-06-01' and '2016-06-08';-- 介于什么之间，包括端点
select * from sales where date in('2016-06-01' ,'2016-06-08');-- 不包括端点
...
select name from product where msrp in (99.95, 199.95);-- 类似between
...
select * from product
where code like 'E%';-- 以E开头的



-- find product code end with Z;
select * from product
where code like '%Z';-- 以Z结尾的

-- find product code contains B;
select * from product
where code like '%B%';-- 中间有的
...
select * from product as p-- 别名
where p.name NOT LIKE '%product%';-- 这个表单下的属性
 ...
 *********************/
-- Calculated columns
select name, msrp*0.9 as a_msrp-- 产品打头，并给新的名字
from product;
